...
/**
 * Tests for GET /api/cron/email-queue-drain — Inngest email-queue-drain trigger.
 *
 * Covers:
...
describe("GET /api/cron/email-queue-drain", () => {
  ...
  function makeGet(opts: { authorization?: string; legacySecret?: string } = {}) {
    ...
    return new Request("https://example.com/api/cron/email-queue-drain", { headers });
  }
  ...
  it("sends Inngest event and returns 200 with valid x-cron-secret", async () => {
    ...
    expect(mockInngestSend).toHaveBeenCalledWith({ name: "cron/email-queue-drain", data: {} });
  });
