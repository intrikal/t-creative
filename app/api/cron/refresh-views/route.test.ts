...
/**
 * Tests for GET /api/cron/refresh-views — Inngest refresh-views trigger.
 *
 * Covers:
...
describe("GET /api/cron/refresh-views", () => {
  ...
  function makeGet(opts: { authorization?: string; legacySecret?: string } = {}) {
    ...
    return new Request("https://example.com/api/cron/refresh-views", { headers });
  }
  ...
  it("sends Inngest event and returns 200 with valid x-cron-secret", async () => {
    ...
    expect(mockInngestSend).toHaveBeenCalledWith({ name: "cron/refresh-views", data: {} });
  });
