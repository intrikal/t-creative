...
export interface ClientHomePageProps {
  ...
  aftercareGuides: AftercareGuide[];
}
...
const QUICK_ACTIONS = [
  ...
  { label: "Photos", icon: Camera, href: "/dashboard/my-photos" },
    ...
    <div className="p-4 md:p-6 lg:p-8 space-y-4">
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-muted mt-0.5">
              You&apos;re {setupProgress} done — complete your profile to unlock all features.
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-5 gap-4">
        ...
        <div className="xl:col-span-2 space-y-4">
            ...
            <CardContent className="px-5 pb-4 pt-2">
              {recentMessages.length === 0 ? (
                <div className="py-6 text-center">
                  <MessageSquare className="w-5 h-5 text-muted/30 mx-auto mb-1.5" />
                  <p className="text-xs text-muted">No new messages</p>
                </div>
              ) : (
                <div className="space-y-2">
                  {recentMessages.map((msg) => (
                    ...
                    >
                      ...
                      <p className="text-[11px] text-muted mt-0.5 truncate">{msg.body}</p>
                    </Link>
                  ))}
                </div>
              )}
          ...
          {/* Aftercare guides */}
          {aftercareGuides.length > 0 && (
            <Card className="gap-0">
              <CardHeader className="pb-0 pt-5">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <BookOpen className="w-4 h-4 text-[#5b8a8a]" />
                    <CardTitle className="text-sm font-semibold">Aftercare</CardTitle>
                  </div>
                  <Link
                ...
                <div className="space-y-1.5">
                  {aftercareGuides.map((guide) => (
                    <Link
                      key={guide.id}
                      href="/dashboard/aftercare"
                      className="flex items-center gap-2.5 py-2 px-2.5 rounded-lg hover:bg-surface transition-colors group"
                    >
                      <HeartHandshake className="w-3.5 h-3.5 text-[#5b8a8a] shrink-0" />
                      <span className="text-xs font-medium text-foreground truncate">
                        {guide.title}
                      </span>
                      <ChevronRight className="w-3 h-3 text-muted/0 group-hover:text-muted ml-auto shrink-0 transition-colors" />
                    </Link>
                  ))}
                </div>
              </CardContent>
