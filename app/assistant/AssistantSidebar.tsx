...
const NAV_GROUPS: NavGroup[] = [
  {
    label: "Daily",
    items: [
      { href: "/assistant", label: "Overview", icon: LayoutDashboard },
      { href: "/assistant/schedule", label: "Schedule", icon: CalendarRange },
      { href: "/assistant/calendar", label: "Calendar", icon: CalendarDays },
      { href: "/assistant/bookings", label: "Bookings", icon: CalendarCheck },
      { href: "/assistant/messages", label: "Messages", icon: MessageSquare },
    ],
  },
  {
    label: "Work",
    items: [
      { href: "/assistant/clients", label: "My Clients", icon: Users },
      { href: "/assistant/services", label: "Services", icon: Scissors },
      { href: "/assistant/aftercare", label: "Aftercare", icon: HeartHandshake },
    ],
  },
  {
    label: "Growth",
    items: [
      { href: "/assistant/earnings", label: "Earnings", icon: DollarSign },
      { href: "/assistant/training", label: "Training", icon: GraduationCap },
      { href: "/assistant/reviews", label: "Reviews", icon: Star },
    ],
  },
];
...
function useIsActive() {
  ...
  return (href: string) => (href === "/assistant" ? pathname === href : pathname.startsWith(href));
}
      ...
      <aside className="fixed top-0 left-0 bottom-0 w-56 hidden lg:flex flex-col bg-background border-r border-border z-40">
        ...
        <div className="px-4 h-12 flex items-center gap-2.5 border-b border-border shrink-0">
          ...
          <div>
            ...
            <p className="text-[9px] text-muted mt-0.5 leading-none">Assistant</p>
          </div>
        </div>
