...
/**
 * StepAdminHours — step 6 of the admin onboarding wizard.
 *
 * ## Responsibility
 * Lets the admin mark specific calendar dates as "open for bookings" and
 * configure their working hours — default start/end time, buffer between
 * appointments, and an optional lunch break.
 *
 ...
 * - Month navigation (`monthOffset` state) lets the admin plan weeks or months ahead.
 * - Past dates are disabled (cannot be selected).
 *
 * ## Per-day overrides
 * When the admin selects a date, an `AnimatePresence`-animated override panel
 * slides in showing that date's custom start/end times (defaults to the global
 * defaults). Changing these times writes to `dayOverrides`. Clicking "reset"
 * removes the override (reverting to defaults). Clicking the ✕ closes the panel
 * without removing the date selection.
 *
 * ## JSON string encoding — why refs instead of setState
 * TanStack Form stores `workingHours.selectedDates` and `workingHours.dayOverrides`
 * as JSON strings (to keep the form schema flat and serializable). This component
 * maintains local `Set` and `Record` state that mirrors the form values, and syncs
 * them to the form via `form.setFieldValue(...)` after every mutation.
 *
 * The key subtlety: `selectedDatesRef` and `dayOverridesRef` hold the current
 * values synchronously. This avoids calling `form.setFieldValue` inside a React
 * `setState` updater — which would trigger a "Cannot update a component while
 * rendering" warning — because callbacks like `toggleDate` are called from event
 * handlers, not render functions.
 *
 * ## Props
 * @prop form - the TanStack Form instance (AdminOnboardingForm)
 * @prop onNext - advances to step 7
...
export function StepAdminHours({ form, onNext, stepNum }: StepProps) {
  ...
  // Month navigation — 0 = current month, can go forward indefinitely
  ...
  // Calendar state — synced to form fields as JSON strings
  const [selectedDates, setSelectedDates] = useState<Set<string>>(() => {
    try {
      const raw = form.getFieldValue("workingHours.selectedDates");
  ...
  const [dayOverrides, setDayOverrides] = useState<Record<string, DayOverride>>(() => {
    try {
      const raw = form.getFieldValue("workingHours.dayOverrides");
      return JSON.parse(raw || "{}") as Record<string, DayOverride>;
    } catch {
      return {};
    }
  });

  // Refs so we can read current values synchronously in callbacks without
  // calling form.setFieldValue inside a setState updater (which React warns against).
  ...
  const toggleDate = useCallback(
    (key: string) => {
      ...
      form.setFieldValue("workingHours.selectedDates", JSON.stringify([...next].sort()));
      ...
      if (removing) {
        ...
        form.setFieldValue("workingHours.dayOverrides", JSON.stringify(newOverrides));
  ...
  const updateOverride = useCallback(
    (key: string, override: DayOverride | null) => {
      ...
      form.setFieldValue("workingHours.dayOverrides", JSON.stringify(next));
    },
    [form],
  );
    ...
    <div className="space-y-3">
      ...
      >
        ...
        <h1 className="text-xl font-semibold text-foreground leading-snug">
          {firstName ? `When are you available, ${firstName}?` : "Set your availability."}
        </h1>
      ...
      >
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <form.Field name="workingHours.defaultStartTime">
            {(field) => (
              <select
                value={field.state.value || "10:00"}
          ...
          <form.Field name="workingHours.defaultEndTime">
            {(field) => (
              <select
                value={field.state.value || "19:00"}
                onChange={(e) => field.handleChange(e.target.value)}
                onFocus={() => (inputFocusedRef.current = true)}
                onBlur={() => {
                  inputFocusedRef.current = false;
        ...
        {/* Buffer */}
        <div className="flex items-center gap-2 flex-wrap">
          <span className="text-xs text-foreground/45 w-12 shrink-0">Buffer</span>
          <form.Field name="workingHours.appointmentGap">
            {(field) => (
        ...
        {/* Lunch break */}
        <form.Field name="workingHours.lunchBreak">
          {(lunchField) => (
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {lunchField.state.value && (
                <>
                  ...
                  <form.Field name="workingHours.lunchStart">
                    {(startField) => (
                      <select
                  ...
                  <form.Field name="workingHours.lunchDuration">
                    {(durField) => (
                      <div className="flex gap-1">
                        {LUNCH_DURATION_OPTIONS.map((l) => (
                          <button
                            key={l.value}
                            type="button"
              ...
              <button
                ...
                onClick={() => {
                  if (isSelected) {
                    ...
                    // Tap unselected = select + auto-open customize panel
                    toggleDate(key);
                    setExpandedDate(key);
                  }
                }}
        ...
        <AnimatePresence>
          {expandedDate && (
            ...
            >
              <form.Subscribe
                selector={(s) => ({
                  defaultStart: s.values.workingHours.defaultStartTime,
                  defaultEnd: s.values.workingHours.defaultEndTime,
                })}
              >
                {({ defaultStart, defaultEnd }) => {
                  const override = dayOverrides[expandedDate];
                  const startVal = override?.startTime || defaultStart || "10:00";
                  const endVal = override?.endTime || defaultEnd || "19:00";
                  return (
        ...
        <p className="text-xs text-muted/45 max-w-[340px]">
          {selectedCount > 0 ? (
            <>
              ...
              {selectedCount === 1 ? "day" : "days"} open — tap again to remove, or set custom hours
              below
            </>
          ) : (
