...
/**
 * Tests for GET /api/cron/birthday-promos — Inngest birthday-promos trigger.
 *
 * Covers:
...
const mockWithCronMonitoring = vi.fn().mockImplementation(
...
describe("GET /api/cron/birthday-promos", () => {
  let GET: (request: Request) => Promise<Response>;

  beforeEach(async () => {
    vi.clearAllMocks();
    mockInngestSend.mockResolvedValue({ ids: ["evt-abc"] });
    mockWithCronMonitoring.mockImplementation(
      async (_name: string, fn: () => Promise<unknown>) => {
        const result = await fn();
        return new Response(JSON.stringify({ ok: true, ...(result as object) }), {
          status: 200,
    ...
    vi.doMock("@/lib/cron-monitor", () => ({ withCronMonitoring: mockWithCronMonitoring }));
  ...
  function makeGet(opts: { authorization?: string; legacySecret?: string } = {}) {
    ...
    return new Request("https://example.com/api/cron/birthday-promos", { headers });
  }
  ...
  it("sends Inngest event and returns 200 with valid x-cron-secret", async () => {
    ...
    expect(mockInngestSend).toHaveBeenCalledWith({ name: "cron/birthday-promos", data: {} });
  });
