...
/**
 * Tests for GET /api/cron/membership-reminders — Inngest membership-reminders trigger.
 *
 * Covers:
...
describe("GET /api/cron/membership-reminders", () => {
  ...
  function makeGet(opts: { authorization?: string; legacySecret?: string } = {}) {
    ...
    return new Request("https://example.com/api/cron/membership-reminders", { headers });
  }
  ...
  it("sends Inngest event and returns 200 with valid x-cron-secret", async () => {
    ...
    expect(mockInngestSend).toHaveBeenCalledWith({ name: "cron/membership-reminders", data: {} });
  });
