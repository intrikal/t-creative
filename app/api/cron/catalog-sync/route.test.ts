...
/**
 * Tests for GET /api/cron/catalog-sync — Inngest catalog-sync trigger.
 *
 * Covers:
...
describe("GET /api/cron/catalog-sync", () => {
  ...
  function makeGet(opts: { authorization?: string; legacySecret?: string } = {}) {
    ...
    return new Request("https://example.com/api/cron/catalog-sync", { headers });
  }
  ...
  it("sends Inngest event and returns 200 with valid x-cron-secret", async () => {
    ...
    expect(mockInngestSend).toHaveBeenCalledWith({ name: "cron/catalog-sync", data: {} });
  });
