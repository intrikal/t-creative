  ...
  try {
    switch (eventType) {
      ...
      case "invoice.payment_made":
        result = await handleInvoicePaymentMade(event.data);
        syncStatus = "success";
        break;
      default:
...
async function handleInvoicePaymentMade(
  ...
  // Find the pending payment record we created when the Square Invoice was published
  ...
  // Fetch the invoice from Square to get payment details
  ...
  try {
    ...
    // The invoice's payment_requests[0] contains the payment request ID
    // which can be used to find the payment
    const paymentRequestId = invoice?.paymentRequests?.[0]?.uid;
    receiptUrl = invoice?.publicUrl ?? null;

    // Try to get the actual payment ID from the invoice
    if (invoice?.orderId) {
      try {
        ...
        // Non-fatal — we can still mark as paid without the payment ID
      }
    }
  } catch {
    // Non-fatal — mark paid even if we can't fetch invoice details
  }
